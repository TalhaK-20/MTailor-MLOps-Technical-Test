...
def create_test_image_base64(image_type: str = "gradient") -> str:
...
class RemoteModelTester:
    ...
    def test_api_connectivity(self) -> bool:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info("Health endpoint not found, testing main endpoint...")
    ...
    def test_main_endpoint(self) -> bool:
        try:
            ...
            test_image = create_test_image_base64("gradient")
            if not test_image:
                return False
    ...
    def test_single_prediction(self, image_type: str, include_extras: bool = False) -> Dict[str, Any]:
        ...
        try:
            # Create test image
            image_base64 = create_test_image_base64(image_type)
            if not image_base64:
                return {
            ...
            if response.get("status") == "success":
                ...
                logger.info(
                    f"{image_type}: {result['predicted_class']} ({result['confidence']:.4f}) [{response_time:.3f}s]")
                return result
            else:
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "average_response_time": np.mean(
                [r.get('response_time', 0) for r in successful_tests]) if successful_tests else 0,
        ...
        for i, result in enumerate(self.test_results, 1):
            ...
            if result['status'] == 'success':
                ...
                print(
                    f"{i:2d}. {status_icon} {image_type:<15} -> {pred_class:<20} ({confidence:.4f}) [{resp_time:.3f}s]")
            else:
